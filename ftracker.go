...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	var answer float64
	answer = ((float64(runningCaloriesMeanSpeedMultiplier) * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / float64(mInKm) * duration * float64(minInH))
	return answer
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	var answer float64
	answer = ((walkingCaloriesWeightMultiplier + math.Pow((meanSpeed(action, duration)*kmhInMsec), 2)/(height/float64(cmInM))*walkingSpeedHeightMultiplier) * weight * duration * float64(minInH))
	return answer
}
